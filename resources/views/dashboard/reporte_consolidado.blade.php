...
@section('title', 'Reporte Consolidado (Ventas + TXD)')
  ...
  <div class="card filter-card mb-4">
    <div class="card-header">
      <div class="header-top">
        ...
        <div>
          <h5>Reporte Consolidado (Ventas + TXD)</h5>
        </div>
...
var sortables   = [];
var LS_KEY      = 'pivot_consolidado_cfg_v4';
...
/* ══════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════ */
var FILTER_DIMS = [
  { key:'Origen',     param:'origenes',    label:'Origen' },
...
var FILTER_OPTIONS = {};
var LS_FILTERS_KEY = 'pivot_consolidado_filters_v1';
...
function isTxdActive(){
  var o = FILTERS['Origen'];
  return o.size === 0 || o.has('TXD');
}

function updateSiCols(){
  if(!gridDetalle) return;
  var show = isTxdActive();
  gridDetalle.setColumnVisible('vta_si26', show);
  gridDetalle.setColumnVisible('vta_si25', show);
}
...
var DET_COLS = [
  ...
  { field:'vta_si26',   headerName:'Vta SI 26',     width:115, valueFormatter:mFmt, hide:true },
  { field:'vta_si25',   headerName:'Vta SI 25',     width:115, valueFormatter:mFmt, hide:true },
];
...
async function loadDetalle(){
  try {
    var r    = await fetch('/dashboard/reporte-consolidado/detalle?'+filterParams());
...
function buildMergedData(act, hst, metas, stock){
  ...
  var actMerged = act.map(function(row){
    return Object.assign({}, row, {vta25:0, gm25:0, unds25:0, meta_vta:0, vta_si25:0});
  });
  var hstMerged = hst.map(function(row){
    return {
      ...
      'meta_vta': 0,
      'vta_si26': 0, 'vta_si25': row['vta_si25'] || 0,
      '_isHst': true
    };
  });
...
async function loadPivotData(){
  ...
  detLoaded = false;
  try {
    var r    = await fetch('/dashboard/reporte-consolidado/pivot?'+new URLSearchParams({ini:activeIni,fin:activeFin}));
...
/* ══════════════════════════════════════════════════════════
   PANEL COLAPSABLE
   ══════════════════════════════════════════════════════════ */
var LS_PANEL_KEY = 'pivot_consolidado_panel_collapsed';
...
/* ══════════════════════════════════════════════════════════
   AUTO-ACTUALIZACIÓN
   ══════════════════════════════════════════════════════════ */
var LS_LIVE_KEY = 'pivot_consolidado_live_ms';
var liveTimer = null;
